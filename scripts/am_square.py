...
def main():
    ...
    beam_sim = SquareRobotSim(data_processor)
    ...
    # AM for ls
    act_max_params = ActMaxParams(nn.BCEWithLogitsLoss(), 1.0e-1, 100, 0.05)
    ...
    A = np.zeros([vae_params.no_classifier_nodes, vae_params.no_classifier_nodes])
    graph.A = A
    ...
    # Graph Target
    graph.add_hand("robot_left_hand", "square_beam_1")
    graph.add_hand("robot_right_hand", "square_beam_2")
    graph_target = torch.tensor([graph.A], dtype=torch.float32).to(vae_params.device)
