...
def graph_to_grad_weight(out_graph: torch.tensor, state_dim: int):
    return torch.kron(torch.diag(out_graph.sum(1) > 0.5).type(torch.float32), torch.eye(state_dim, device=out_graph.device))
...
def main():
    ...
    # AM for ls
    act_max_params = ActMaxParams(nn.BCEWithLogitsLoss(), 0.5e-3, 20, 0.2)
    ...
    x_pred_prev = out_pred.x_pred.clone()
    
    init_pin_pose = x_pred_prev[:, -5:].clone()    
    vae_inputs = BeamVaeInputs()
    
    latents.mu = latents.z.clone()
        ...
        while viewer.is_running():
            while torch.norm(loss) > act_max_params.stop_criterion and counter < act_max_params.max_iters:
                ...
                with torch.no_grad():
                    ...
                    latent_list.append(latents.z.clone().detach().cpu().numpy().squeeze())
                    
                    # if counter % 4 == 3:
                    #     out_pred = model.decoder(latents, None)
                    #     out_graph = model.classifier(latents.z)
                        
                    #     grad_weight = graph_to_grad_weight(torch.sigmoid(out_graph[0, :, :]).round(),
                    #                                         vae_params.state_dim).unsqueeze(0)
                        
                    #     # out_pred.x_pred = x_pred_prev + torch.matmul(grad_weight, (out_pred.x_pred - x_pred_prev).squeeze())
                        
                    #     vae_inputs.x_in = out_pred.x_pred
                    #     latents = model.encoder(vae_inputs)
                            
                    #     # Set latents z to mu
                    #     latents.z = latents.mu.clone()
                    
                    closed_loop_latent_traj.append(latents.z.clone().detach().cpu().numpy())
            ...
            closed_loop_latent_traj = np.vstack(closed_loop_latent_traj)
            ...
            plt.figure()
            plt.plot(closed_loop_latent_traj[:, latent_dims[0:3]], '*')
            plt.plot(latent_traj[1:, latent_dims[0:3]])
