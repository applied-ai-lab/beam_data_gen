...
def main():
    ...
    no_samps = 1000
    x, y = latent_inspector.sample_latent_values_from_unit_circle_2d(radius=3.0, no_samps=no_samps)
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running():
            while torch.norm(loss) > termination_criteria:
                ...
                loss = loss_func(out_graph, graph_target)
                loss_lst.append(loss.detach().cpu().numpy().squeeze())
                ...
                latents.z[0, latent_dims[0]] = z_polar[0] * torch.cos(z_polar[1])
                latents.z[0, latent_dims[1]] = z_polar[0] * torch.sin(z_polar[1])
            ...
            # Plot latents
            axis = latent_inspector.plot_latents(x, y, graphs_for_plotting)
            axis.plot(latent_traj[:, latent_dims[0]], latent_traj[:, latent_dims[1]], 'k*')
            axis.set_xlabel("z0")
            axis.set_ylabel("z1")
            axis.set_title("Latent Space and Trajectory")
