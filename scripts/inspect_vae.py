    ...
    with torch.no_grad():
        ...
        with mujoco.viewer.launch_passive(m, d) as viewer:
            ...
            while viewer.is_running():
                ...
                latents.z = torch.randn([1, vae_params.latent_dim], dtype=torch.float32).to(vae_params.device)
                ...
                # Set position
                d.qpos[0:3] = denorm_out[0, 0:3].cpu().numpy()
                d.qpos[7:10] = denorm_out[0, 5:8].cpu().numpy()
                d.qpos[14:17] = denorm_out[0, 9:12].cpu().numpy()
                
                # Set orientation
                l1_z = R.from_euler("xyz", [0, 0, denorm_out[0, 3].cpu().numpy()])
                l2_z = R.from_euler("xyz", [0, 0, denorm_out[0, 8].cpu().numpy()])
                pa_z = R.from_euler("xyz", [0, 0, denorm_out[0, 13].cpu().numpy()])
